...
import { getLineageData } from '@/data/lineage';
...
export default function LineageView() {
  ...
  return (
    <div className="space-y-5 h-[calc(100vh-160px)] flex flex-col">
      ...
      <div className="flex-1 glass-card gradient-border overflow-hidden relative fade-in-up flex flex-col">
      ...
      {selectedNode && (
        <div className="fixed inset-0 z-40 flex items-end md:items-center justify-end md:justify-center p-4 pointer-events-none">
          <div
            className="pointer-events-auto glass-card gradient-border w-full md:max-w-md fade-in-up overflow-hidden"
          ...
          >
            <div className="flex items-start justify-between p-5 border-b border-white/[0.06]">
              <div>
                ...
                <h3 className="font-display font-semibold text-lg text-white">{selectedNode.name}</h3>
              </div>
              <button
                onClick={() => setSelectedNode(null)}
                className="w-8 h-8 rounded-lg flex items-center justify-center text-slate-500 hover:text-slate-200 hover:bg-white/[0.06]"
              >
                <X className="w-4 h-4" />
              </button>
            </div>
              ...
              {selectedAsset ? (
                <>
                  ...
                  <div className="flex gap-2 pt-2">
                    ...
                    <Link
                      to={`/lineage/${selectedNode.id}`}
                      onClick={() => setSelectedNode(null)}
                      className="btn-ghost text-xs flex-1 text-center py-2.5 !bg-violet-500/10 !border-violet-500/30 !text-violet-300"
                    >
                      以其为中心
                    </Link>
                  </div>
                ...
                <div className="text-center py-4 text-slate-500 text-xs">暂无详细信息</div>
              )}
            </div>
          </div>
        </div>
